...
## 2026-09-10 · 模板、模型与引擎职责边界重述

- **目标：** 回应“模板究竟如何设置、模型是不是只负责填充、是否本质为生成图片，以及这对我们有什么意义”的连续追问，把已经完成的源码分析压缩成外部读者可独立理解的首页摘要。
- **源码复核：** `SKILL.md` 明确 v1—v3 要锁定模板视觉、结构、元素数量与媒体槽，只替换可见文案和允许属性；v4 输出结构化 composition，不接收任意 raw CSS。`BespokeSlide.jsx` 将它限制在 12×8 网格、最多 32 个元素，`theme-profile-core.mjs` 定义 9 类构图，各主题 adapter 再提供 recipe。PPTX 导出器遍历 DOM 生成原生文字、形状与图片，只对复杂区域截图回退。
- **真实配置复核：** REAL RUN 02 使用的 `theme03_page076` manifest 预先声明深色模式、0—2 张图、横/纵布局、0—3 个 chips、figure/callout 显隐与 blue/lime 强调色；运行配置只设置 1 张图、3 个 chips，并用 `contentMap` 将标题、摘要和指标绑定到 kicker、lead 与 chips。
- **更新：** 重写根研究索引、项目 README 执行摘要/当前结论、Web 首屏、机制实验台、职责分工、扩展与采用门槛；新增摘要契约测试，明确“预制组件 + Agent 理解/有限编排 + 确定性装配/导出”，并写明 Dashi 不是整页生图。
- **发布：** 摘要提交 `4002a73206e52d09d33e322b707edb6daee6eb21` 已推送 `origin/main`；Repository checks run `34447034242` 与 Deploy research site run `34447034271` 均通过。canonical Web 在 1440×1000、768×1024、390×844 三档读取到新摘要，0 overflow、0 console error，5 个公开下载/报告链接均为 HTTP 200。
- **判断：** 模型并非仅做简单字段替换：它仍负责材料理解、叙事、文案、布局意图与 v4 结构化 composition；但视觉自由被预制模板、网格和主题 recipe 明确限制。现成 1,020 个布局提供效率，不构成我们的独有资产；真正值得吸收的是把企业品牌、常用页型、容量、字段绑定和 QA 变成机器可执行模板系统。
- **边界：** 锁定的 `0.4.11` 分发版未暴露成熟的 PPTX 模板导入能力或稳定外部主题插件；深度品牌定制需要 fork / 源码集成，或另建模板扩展层。图片可由外部生成模型提供后插入，但不是 Dashi 的页面生成原理。
- **下一步：** 若决定采用，下一轮不再继续堆现成模板样例，而应选 10—20 个高频企业页型，制作一个最小自有主题，并以品牌精度、内容溢出、原生编辑率和返工量验收。
