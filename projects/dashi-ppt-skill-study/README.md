...
> 第 3 个研究子项目。Dashi PPT Skill 本质是“预制主题/页面组件 + Agent 内容理解与有限编排 + 确定性装配/导出”，不是让大模型从空白画 PPT，也不是把整页生成为图片。本研究解释模板如何定义、运行时如何填充，以及这套方法对组织级演示生产的价值与边界。
...
- **它是什么：** Dashi 不是独立模型，也不是整页生图工具。它把预先写好的主题与页面组件交给宿主 Agent 使用，再用容量约束、`props`、`contentMap`、浏览器渲染和分层导出组成一条本地 PPT 生产线。
- **模板与模型如何分工：** 模板作者预设视觉结构、内容槽位、容量与允许调整项；Agent 理解材料、规划叙事并产生结构化内容；Dashi 负责选模板、绑定字段、渲染、编辑、验证和导出。
- **所谓“定制”：** v1—v3 锁定现成页面结构，只替换可见文字、媒体和允许的属性；v4 允许 Agent 生成结构化 composition，但仍受 12×8 网格、9 类构图和主题 recipe 约束，不是任意 CSS 或像素级自由生成。
- **实测支持什么：** 固定输入和规则可以复现同一 goal；两轮实验均实际生成 HTML/PDF/PPTX，并通过最终 PowerPoint 越界检查。
- **它何时更有价值：** 高频、结构重复、需要同时比较多套方案、需要留下输入/分配/导出证据的组织汇报；长期价值是把自己的品牌和汇报经验编码成可执行模板系统。
...
这里的“模板”不是一张背景图，而是一组可执行的 React / HTML / CSS 页面组件。模板作者在构建期确定字体、颜色、网格、留白、装饰和内容层级，同时声明标题、正文、指标、图片等槽位，规定每个槽位能装多少内容，并把少量可调项暴露为 `props`。运行时，Agent 不重新绘制 v1—v3 页面，而是生成逐页 brief 和 canonical content；Dashi 按容量挑选能装下内容的组件，再用 `contentMap` 把内容字段投影到组件槽位。

锁定版本的 `theme03_page076` 是一个可验证例子：模板预先允许深色模式、0—2 张图片、横/纵排列、0—3 个 chips、图形与 callout 显隐，以及 blue/lime 强调色；[真实运行配置](experiments/real-run-02-brand-media/goal.json)只设置了 1 张图片、3 个 chips，并把演示标题映射到 kicker、摘要映射到 lead、3 个指标映射到 chips。也就是说，设计边界早已写在模板里，Agent 负责理解“放什么”，Dashi 负责执行“放到哪里、是否装得下”。

第 4 个 bespoke 候选比前三个更自由，但自由度同样被工程化：Agent 输出结构化 composition，由 Dashi 内置的 12×8 网格、最多 32 个元素、9 类 composition family 和各主题 recipe 渲染。图片若需要生成，应先由外部图像模型或用户素材提供，再暂存并插入媒体槽；Dashi 自身不负责把整页画成一张图。导出 PPTX 时，浏览器 DOM 会尽量转换为原生文字、形状和图片，复杂区域才使用截图回退。

**对我们的意义：** 直接使用上游 1,020 个版式只能获得效率，不会形成独有能力。更值得沉淀的是自己的企业主题、常用页型、字段与容量契约、图表规则和 QA 门槛，让模型在明确边界内稳定装配。当前 `0.4.11` 分发版没有成熟的“上传 PPTX 即成为主题”能力或稳定外部主题插件接口；深度品牌化需要 fork / 集成源码，或在其上建设自己的模板扩展层，这应作为下一阶段试点的核心工程问题。
...
Dashi PPT Skill 不是独立的大模型，也不是传统 PowerPoint 模板包，而是一套由宿主 Agent 驱动的可编程模板生产线：模板作者先定义视觉结构、内容槽位、容量和可调属性；Agent 负责理解材料、形成逐页叙事和结构化内容；确定性脚本负责版式查询、字段绑定、全稿分配、React/HTML 渲染、浏览器编辑、校验和导出。

锁定版本的版式清单包含 12 个主题、1020 个版式和 8576 个控制项。每个逻辑页共享一份 canonical content，并生成 3 个锁定结构、通过 `props` / `contentMap` 填充的模板候选，以及 1 个受 12×8 网格、9 类构图和主题 recipe 约束的 Agent 定制候选。图片生成是外部可选能力，不是 Dashi 的页面生成本质。HTML 是交互和编辑能力最完整的主产物；PPTX 由浏览器读取实时 DOM 后逐节点转换，无法稳定映射的区域使用截图回退，因此“可编辑”是分层保真而不是所有元素百分之百原生化。

两轮实测支持一个更克制的结论：它的特色主要是可复现、可对照、可审计的生产线，不是独特视觉风格。固定 seed 能稳定复现布局，也能稳定复现不合适的布局；使用现成主题只能提高生产效率，不能自动形成我们的品牌资产。真正值得吸收的是“把品牌、常用页型、容量、字段绑定和 QA 写成机器可执行规则”的方法。严格品牌和原生图表场景中，直接编程基线更可控；高频标准化汇报中，Dashi 的 3+1 候选和验证链更有杠杆。PowerPoint 级视觉 QA 和人工选稿仍不可省。主体采用 AGPL-3.0，但 HTML→PPTX 导出子包为专有组件，不能把整个仓库简单视为可任意拆分复用的纯开源库。
