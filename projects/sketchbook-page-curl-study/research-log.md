...
## 2026-08-31 · E16 采用意义可见性纠偏

- **用户观察：** Revision 7 已经提交，但从原 Revision 6 虹膜快门深链进入时“没看到意义”。线上复核确认该观察成立：旧深链停留在 `panel=explore`；即使直接打开 `panel=surfaces`，页面也只有效果、机制和技术边界，“适用场景 / 对你的意义”仍只存在于 README。
- **根因：** 研究结论已经写入文档，但 Demo 的信息架构把采用价值留在网页之外；内容上下文选择器只能换内容，不能回答“为什么选这种形态”。
