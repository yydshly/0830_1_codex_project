...
3. **工程证据：** 35/35 项 Node 测试通过；Revision 7 浏览器证据覆盖 12 项结构计数、五材质可见差异、8 内容上下文、定向拖动、深链刷新、radio 键盘、退出恢复、1440/768/390、reduced-motion、fallback 与无外部运行时资源。repository validator 在最终发布提交前再次执行。
...
Revision 6 公开基线为 commit `84ae99c`：GitHub Actions `Repository checks` run `33366112690` 与 `Deploy research site` run `33366112730` 均成功。Revision 7 的新 commit、Actions run、Pages HTTP 200 与线上 Chromium 深链证据将在推送后写回；旧 run ID 不冒充本轮发布。完整证据记录在 `research-log.md` 的 E13/E14；本文件不把 CSS 材质、REMIX/HORIZON 或真机性能冒充已完成。
