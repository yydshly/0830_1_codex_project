...
$UpstreamUrl = "https://github.com/MengTo/sketchbook.git"
$UpstreamCommit = "c1e477814c4c9e204452ebf9b298aa13629cbfc2"
$ProjectRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path

if ([string]::IsNullOrWhiteSpace($Destination)) {
    $Destination = Join-Path $ProjectRoot "upstream\sketchbook"
...
if (-not (Test-Path -LiteralPath $Destination)) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "git init failed for $Destination" }
}
...
if ($LASTEXITCODE -ne 0) { throw "Could not list Git remotes in $Destination" }
...
if ($LASTEXITCODE -ne 0) { throw "Could not fetch locked upstream commit $UpstreamCommit" }
...
Write-Output "MengTo/sketchbook upstream is ready."
