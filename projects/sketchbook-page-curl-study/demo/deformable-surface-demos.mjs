...
export const SURFACE_DEFINITIONS = Object.freeze({
  'label-peel': definition({
    ...
    scenarios: Object.freeze(['商品包装', '优惠揭示', '前后对比']),
    meaning: '在不离开当前画面的情况下，让用户主动揭开第二层信息，可替代弹窗或跳页。',
    accent: '#ef674f',
    action: '揭开标签',
    kind: 'release',
    progressLabel: '剥离行程',
  ...
  'fold-map': definition({
    ...
    scenarios: Object.freeze(['旅行路线', '园区导航', '展览导览']),
    meaning: '把阅读顺序变成空间路线，让位置、步骤与进度形成直观关系。',
    accent: '#d8873f',
    action: '展开下一折',
    kind: 'cycle',
    maxStep: 7,
  ...
  'split-curtain': definition({
    ...
    scenarios: Object.freeze(['产品发布', '品牌首页', '章节转场']),
    meaning: '用可操控的开场建立焦点与仪式感，无需依赖预制视频。',
    accent: '#b64f68',
    action: '拉开幕布',
    kind: 'toggle',
    progressLabel: '中央开场',
  ...
  'comparison-blind': definition({
    ...
    scenarios: Object.freeze(['改造前后', '时间变化', '材质／数据对照']),
    meaning: '让两个状态保持相同空间坐标，降低用户来回比对的认知成本。',
    accent: '#397e9e',
    action: '切换到背面状态',
    kind: 'toggle',
    progressLabel: '百叶换面',
  ...
  'accordion-timeline': definition({
    ...
    scenarios: Object.freeze(['品牌历史', '项目里程碑', '流程叙事']),
    meaning: '把较长序列压缩成可逐段展开的结构，同时保留已阅读上下文。',
    accent: '#9a659f',
    action: '展开下一事件',
    kind: 'cycle',
    maxStep: 6,
  ...
  'material-wall': definition({
    ...
    scenarios: Object.freeze(['家居配置', '汽车选配', '服装／电商']),
    meaning: '让用户在同一产品状态下比较多个方案，直接辅助选择与决策。',
    accent: '#a86d43',
    action: '选择下一块样本',
    kind: 'cycle',
    maxStep: 7,
  ...
  'box-net': definition({
    ...
    scenarios: Object.freeze(['包装设计', '产品拆解', '组装说明']),
    meaning: '在立体结构与平面说明之间建立可操作的桥梁，兼顾展示和解释。',
    accent: '#4b8e77',
    action: '展开包装盒',
    kind: 'toggle',
    progressLabel: '盒体展开',
  ...
  portal: definition({
    ...
    scenarios: Object.freeze(['游戏入口', '沉浸叙事', '创意场景切换']),
    meaning: '把普通页面跳转变成可感知的空间阈值，让场景变化更连续。',
    accent: '#6559bd',
    action: '穿过表面',
    kind: 'toggle',
    progressLabel: '穿越阈值',
  ...
  'rollup-poster': definition({
    ...
    scenarios: Object.freeze(['活动海报', '公告揭示', '长图解说']),
    meaning: '用渐进揭示控制信息节奏，在有限版面中容纳第二层内容。',
    accent: '#d4604c',
    action: '卷起海报',
    kind: 'toggle',
    progressLabel: '卷轴行程',
  ...
  'radial-fan': definition({
    ...
    scenarios: Object.freeze(['色卡', '材质册', '规格／菜单选择']),
    meaning: '把大量选项压缩到单一锚点周围，形成更具实物感的选择器。',
    accent: '#cc8545',
    action: '选择下一扇片',
    kind: 'cycle',
    maxStep: 9,
  ...
  'tearoff-coupon': definition({
    ...
    scenarios: Object.freeze(['优惠领取', '票券凭证', '下载／分享']),
    meaning: '从“显示信息”扩展到“取得对象”，验证吸附、阈值和拓扑脱离。',
    accent: '#db7043',
    action: '沿穿孔撕取',
    kind: 'release',
    progressLabel: '撕取行程',
  ...
  'data-ribbon': definition({
    ...
    scenarios: Object.freeze(['指标趋势', '业务流程', '实时数据叙事']),
    meaning: '把数据节点和变化顺序转成可操控路径，适合引导式讲解。',
    accent: '#417fbe',
    action: '推进下一数据点',
    kind: 'cycle',
    maxStep: 7,
